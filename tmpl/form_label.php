...
<div id="contactus-form<?php if (isset($module->id)){echo $module->id;};?>" class="contactus-form contactus-form<?php if (isset($module->id)){echo $module->id;};?> <?php if (isset($fields->margin)){echo "contactus-".$fields->margin;}; ?>">
	<form  class="reg_form"  action="<?php echo JFactory::getURI();?>" onsubmit="joomly_analytics(<?php echo $module->id;?>);return contactus_validate(this);" method="post" enctype="multipart/form-data" novalidate>
		<div>
					...
					{	
						switch ($f->type){
							case "Input":
								...
								<div class="joomly-contactus-div">
									<label class="contactus-date-label" ><?php echo $f->title;if ($f->required == 1){echo '*';};?></label>
						...
						<?php break; 
							case "Phone":
								...
								<div class="joomly-contactus-div">
									<label class="contactus-date-label" ><?php echo $f->title;if ($f->required == 1){echo '*';};?></label>
									<input type="tel" <?php if (isset($fields->mask) && strlen($fields->mask) > 0) {echo 'onkeypress="joomlyHandleMask(event, \'' . $fields->mask . '\')"';};?> placeholder="<?php echo $f->title;if ($f->required == 1){echo '*';};?>" class="contactus-fields <?php echo $f->name.$module->id;?>" data-id="<?php echo $k.$module->id;?>" name="values[<?php echo $f->name;?>]" <?php if ($f->required == 1){echo "required";}; ?> value="<?php if (isset($data[$f->name])){echo $data[$f->name];};?>" />
								</div>
						<?php break; 
							case "Textarea":
								if ($f->dependency != "")
						...
						<?php break;
							...
							case "Select":
								...
								<div class="joomly-contactus-div select-container">
									<label class="contactus-date-label" ><?php echo $f->title;if ($f->required == 1){echo '*';};?></label>
									<select class="contactus-select contactus-fields <?php echo $f->name.$module->id;?>" data-id="<?php echo $k.$module->id;?>" name="values[<?php echo $f->name;?>]" <?php if ($f->required == 1){echo "required";}; ?>>
