					...
					{	
						...
						<?php break; 
							case "Phone":
								...
								<div class="joomly-contactus-div">
									<input id="phone" type="tel" pattern="(\+?\d[- .\(\)]*){5,15}" placeholder="<?php echo $f->title;if ($f->required == 1){echo '*';};?>" class="contactus-fields <?php echo $f->name.$module->id;?>" data-id="<?php echo $k.$module->id;?>" name="<?php echo $f->name;?>" <?php if ($f->required == 1){echo "required";}; ?> value="<?php if (isset($data[$f->name])){echo $data[$f->name];};?>" />
								<script src="https://pro-spec.ru/components/com_jshopping/addons/addon_onestepcheckout/js/jquery.maskedinput.min.js" type="text/javascript"></script>
								<script>
jQuery(function($){
